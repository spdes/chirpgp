"""
Analysis the social call of a bat Eptesicus nilssonii.
...
# Create the model
params = jnp.array([0.2, 1., 1., 0.5, 5., 1.])
...
nperseg = 400
noverlap = nperseg - 1
freqs, sp_ts, Sxx = scipy.signal.spectrogram(ys, fs, nperseg=nperseg, noverlap=noverlap, mode='psd', nfft=2048)
sp_ts = sp_ts + ts[0]
...
plt.savefig('nilssonii.png', transparent=True)
plt.show()
