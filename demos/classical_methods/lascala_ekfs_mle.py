"""
Use the model introduced by Barbara F. La Scala. Essentially the same as with Donald L. Snyder.

In addition to their works, model parameters here are optimised via MLE, and we also use a bijection to ensure the
positivity of IF.
"""
import math
import jax
...
from chirpgp.models import g, g_inv, build_lascala_model
...
for mag in [constant_mag(1.),
    ...
    # From left to right, they are, delta, ell, sigma, m0_1
    init_theta = g_inv(jnp.array([0.1, 1., 1., 7.]))

    # Objective function
    @jax.jit
    def obj_func(theta: jnp.ndarray):
        _, _, m_and_cov, m0, P0, H = build_lascala_model(g(theta))
    ...
    _, _, m_and_cov, m0, P0, H = build_lascala_model(opt_params)
