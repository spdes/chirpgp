...
for mc in range(num_mcs):
    ...
    for mag, name in zip((constant_mag(1.), damped_exp_mag(0.3), random_ou_mag(1., 1., key_for_ou)),
        ...
        def obj_func(theta: jnp.ndarray):
            F, Sigma, m0, P0, h = build_kpt_chirp_model(g(theta), fs, num_harmonics=num_harmonics)
        ...
        if opt_state.success:
            ...
            F, Sigma, m0, P0, h = build_kpt_chirp_model(opt_params, fs, num_harmonics=num_harmonics)
        ...
        file_name = f'./results/harmonic_kpt_mle_{name}_{mc}.npz'
